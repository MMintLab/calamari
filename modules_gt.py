...
class contact_mlp_decoder(nn.Module):
    def __init__(self, dim_ft, last_activation = False, dim_out = 150**2):
        ...
        nn.init.kaiming_normal_(self.l3_1.weight, a=0.0, nonlinearity='relu', mode='fan_in')
        nn.init.kaiming_normal_(self.l3_2.weight, a=0.0, nonlinearity='relu', mode='fan_in')
        nn.init.kaiming_normal_(self.l3_3.weight, a=0.0, nonlinearity='relu', mode='fan_in')
    ...
    def forward(self, x):
        ...
        x_cnt = self.sigmoid(self.l3_1(x)).reshape(-1, self.dim_out_sqrt, self.dim_out_sqrt)
