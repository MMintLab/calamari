...
class ContactEnergy():
    def __init__(self, log_path, test_idx = (30, 37)):
        ...
        # Load model from log.
        pretrained = torch.load(args.logdir, map_location = torch.device('cpu'))

        self.policy_pt = policy(dim_in=224, dim_out=dimout, image_size=224,
                                Config=self.Config).cuda()

        self.policy_pt.tp_transformer.load_state_dict(
            pretrained["segment_emb"]
        )

        self.policy_pt._image_encoder.load_state_dict(pretrained["image_encoder"])

        self.policy_pt.vl_transformer_encoder.load_state_dict(
    ...
    def get_energy_field(self):
        ...
        self.write_tensorboard_test(contact_histories, contact_histories_ovl, tot_loss)
    ...
    def write_tensorboard_test(self, contact, contact_ovl):
        contact = torch.stack(contact).unsqueeze(3)
        contact_ovl = torch.stack(contact_ovl, dim = 0)

        with self.file_writer.as_default():
            tf.summary.image("contact_test", contact, max_outputs=len(contact), step=0)
            tf.summary.image("contact_ovl_test", contact_ovl, max_outputs=len(contact_ovl), step=0)
...
CE = ContactEnergy( log_path = 'transformer_h_lava_test')
CE.get_energy_field()
