...
class PretrainedPolicy:
    def __init__(self, logdir, model_type, save_result = False):
        ...
        if args.model_type == 'm1':
            ...
            self.policy_pt = policy(self.Config.device, self.Config.dim_ft, dim_out=dimout).cuda() ## 224 or 225

        elif args.model_type == 'm2':
            from language4contact.modules_seq import policy

            self.policy_pt = policy(self.Config.device, self.Config.dim_ft, dim_in = 224, dim_out=dimout).cuda() ## 224 or 225
    ...
    def feedforward(self, rgb_path, txt) -> torch.tensor:
        ...
        if self.model_type == 'm1' :
            ...
            print(torch.amin(cost))
            cv2.imwrite("contact_cost.png", cost[0].detach().cpu().numpy() * 10. )
            contact_round = round_mask(contact.detach().cpu(), thres = 0.1)
            cost_reg_ori_, cost_reg_ori = energy_regularization(cost.detach().cpu(), contact_round, minmax = (0,2.5), return_original = True)
            # cost_reg_ori_ = torch.dstack([cost_reg, contact_round, contact_round])
            ...
            contact = contact[:4, :, :] 
            cost_reg_ori_ = union_img(contact.detach().cpu(), thres = 0.4)

        elif self.model_type == 'm3':
            contact, _, _, out = self.policy_pt(feat, seg_idx)
            cost_reg_ori_ = round_mask(contact[0].detach().cpu(), thres = 0.2).unsqueeze(2)
        ...
        if self.model_type == 'm1':
            contact_ovl = overlay_cnt_rgb(rgb_path[0], cost_reg_ori_.squeeze().detach().cpu(), rgb_image=img)
            cv2.imwrite("contact_ovl.png", contact_ovl.numpy()[:,:,[2,1,0]] )
            cv2.imwrite("contact_ori.png", contact_round.squeeze().detach().cpu().numpy() *255. )
            return contact_ovl

        elif self.model_type == 'm2':
            contact_ovl = overlay_cnt_rgb(rgb_path[0], cost_reg_ori_.squeeze(), rgb_image=img)
...
if __name__ == "__main__":
    ...
    energy_reg = m.feedforward([args.indir], "Use the sponge to clean up the dusts.")
