...
class DatasetTemporal(torch.utils.data.Dataset):
            ...
            for i in tqdm(task[self.mode+"_idx"]):
                ...
                for local_idx in range(len(traj_rgb_fn)):
                    ...
                    for d_idx, aug_idx in enumerate(self.idx_lst):
                        ...
                        aug_idx= np.array(aug_idx)[np.newaxis,:]

                        traj_cnt_img =  augmentation(aug_idx, torch.stack(traj_cnt_img), rgb = False)
                        traj_cnt_img = [traj_cnt_img[0]]

                        visual_sentence, fused_x, vl_mask, tp_mask =  self.input_processing(traj_rgb_lst, txt, task_n, aug_idx = aug_idx, heatmap_folder = heatmap_folder)
    ...
    def __getitem__(self, idx_raw):
        ...
        return   {
                "visual_sentence": self.aug_summary[idx_raw]['visual_sentence'],
                "fused_x" : self.aug_summary[idx_raw]['fused_x'],
