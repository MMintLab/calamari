...
class Config:
    def __init__(self, task=None):
        ...
        self.dataset_config = self.task_confg.task_policy_configs[task]
    ...
    def world_to_camera_batch(
        self, cnt_pts: torch.tensor, mask: torch.tensor, img_size: list
    ) -> np.ndarray:
        """
        ...
        front_pxls = (
            torch.tensor(self.camera_proj).to(self.device).float()
            @ cnt_pts_.clone().detach().to(self.device).float()
        )
        ...
        for b in range(B):
            ...
            imgs[b, front_pxls[b, iidx, 0], front_pxls[b, iidx, 1]] = (
                torch.ones_like(front_pxls[b, iidx, 0]).to(self.device).float()
            )
    ...
    def get_contact_options(self, W):
        ...
        for i in range(W):
            if i == 0:
                ...
                options[:, i] = (np.arange(0, W**6) % (W ** (6 - i - 1))) // (
                    W ** (6 - i - 2)
                )
