...
def energy_regularization(energy, mask=None, minmax=None, return_original=False):
    ...
    if minmax is None:
        max = np.amax(
            energy[iidx, jidx]
        )  # .reshape(mask.shape[0], -1), axis = -1)# [:, np.newaxis, np.newaxis]
        min = np.amin(
            energy[iidx, jidx]
        )  # .reshape(mask.shape[0], -1), axis = -1)#[:, np.newaxis, np.newaxis]
    else:
...
def gradient_img(img, device="cuda"):
    ...
    conv1.weight = nn.Parameter(
        torch.from_numpy(a).float().unsqueeze(0).unsqueeze(0).to(device)
    )
...
def union_img(img_lst, thres=0.5):
    ...
    _, iidx, jidx = np.where(img_lst == 1.0)
...
def overlay_cnt_rgb(rgb_path, uib, uic, Config):
    ...
    if len(cnt_pxls) == 0:
        ...
        cnt_pts = Config.contact_frame_to_world(
            cnt_pxls
        )  # project from desk to front camera image space
        ...
        overlaid[idx[:, 1], idx[:, 0], :] = mapped_cnt * 255.0
...
def overlay_cnt_rgb(rgb_path, cnt_pred, rgb_image=None) -> torch.tensor:
    ...
    rgb[iidx, jidx, :] = uic[iidx, jidx, :] / np.amax(uic) * 255.0
...
def seq_overlay_cnt_rgb(rgb_path, cnt_pred, rgb=None):
    if rgb is None:
        ## open rgb image with cv2
        rgb = cv2.imread(rgb_path)
        rgb = cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)[:, :, :3] * 255.0
    else:
        rgb = cv2.resize(rgb, cnt_pred.shape[:2])
    ...
    uic = cnt_pred.numpy()
    iidx, jidx = np.where(np.sum(uic, axis=-1) != 0)
    rgb[iidx, jidx, :] = uic[iidx, jidx, :] * 255.0
    return torch.tensor(rgb)
