...
class ContactEnergy():
    ...
    def write_tensorboard(self, step, contact, energy, energy2d):
        contact = torch.cat(contact, dim = 0).unsqueeze(3)
        energy = torch.cat(energy, dim = 0)
        with self.file_writer.as_default():
    ...
    def get_energy_field(self, folder_path):
        for i in range (self.Config.epoch):
            ...
            for data in self.DataLoader:
                ...
                gt_score, s_i = loss.fast_trajectory_score(energy, cnt_gt, range(0, cnt_gt.shape[1]), self.Config, return_si= True)
                random_score = loss.faster_score_negative(s_i, self.Config)

                energy_loss_i = gt_score / (gt_score + random_score)  
                # energy_loss_i = random_score / (gt_score + random_score)           
                energy_loss_i = - torch.log( energy_loss_i )
            ...
            if i % 5 == 0 or i == self.Config.epoch -1:
                self.write_tensorboard(i, contact_histories, energy_histories, energy_histories_1d)
